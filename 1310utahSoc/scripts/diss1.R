## processed thru line 25 7/5/12

load("C:/Documents and Settings/Russell/Desktop/VAT/Projects/Research/Dissertation/data/Analytics/dissert.RData")
require(strucchange)
require(ggplot2)
require(forecast)                       #for auto.arima
require(lmtest)                         #for dwtest
## adf.test is from tseries which should be in base

## initial step will be to take original three series and do auto.arima, dwtest, and adf.test on them
## mtoe is Fouquet,
## gdp is composed from snooks 1300-1700, officer from 1700 - 1873
## pop is composed from snooks 1300-1540, mitchell 1540-1801, mitchell 1801-1873

## auto.arima energy in levels
auto.arima(mtoespline,stepwise=FALSE,trace=TRUE)
## dwtest
dwtest(mtoespline~1)
## adf
adf.test(as.vector(mtoespline),alternative='explosive')

## auto.arima energy in log levels
auto.arima(log(mtoespline),stepwise=FALSE,trace=TRUE)
## dwtest
dwtest(log(mtoespline)~1)
## adf
adf.test(as.vector(log(mtoespline)),alternative='explosive')
...
## just to get breakpoints on ln mtoe
plot(mtoesplineln)
...
ggplot(df.wageenergy,aes(x=city,y=wageenergyratio))+
    ...
    opts(title='Source Energy Substitution Ratios \n c. 1700')
#    scale_fill_continuous()

ggsave(file='notes/wage-energy.png')
...
load("C:\\Documents and Settings\\Russell\\Desktop\\VAT\\Projects\\Research\\Conferences\\1009Gothenburg-WS Workshop\\gothenburg.RData")
...
ggplot(df.acres,aes(x=year,y=acres/1000000))+
    ...
    opts(title='Forest Acres Required for English Energy Consumption \n counterfactual CE 1300 - 2008')

ggsave(file='notes/wood.png')
...
china.mtoe <- read.csv('C:/Documents and Settings/Russell/Desktop/VAT/Projects/Research/Conferences/1203EEA/data/china.energy.csv', header=TRUE, stringsAsFactors=FALSE)
...
ggplot(china.mtoe,aes(x=Year,y=acres/1000000))+
    ...
    geom_line(size=1.5,colour='blue')+
    scale_x_continuous(name='Year')+
    scale_y_continuous(name='Million Acres',formatter='comma')+
#    geom_text(1990,2400,label='2.304 billion acres total land area',colour='red')+
    ...
    opts(title='Forest Acres Required for Chinese Energy Consumption \n counterfactual 1978 - 2008')

ggsave(file='notes/china.wood.png')
