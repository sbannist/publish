...
df.allen.wage <- read.csv('data/allen_wages1.csv',header=TRUE,stringsAsFactors=FALSE) #with cities across columns
...
ggplot(df.allen.wage.m, aes(x=Year,y=value,group=City)) +
    geom_line(aes(colour=City),size=2.5)+
    scale_y_continuous(name='grams of silver per day')+
    scale_x_continuous(name='Year')+
    opts(title='World Daily Wage \n grams of silver', legend.position=c(0.21,0.64),
         legend.background = theme_rect(col = 0))

ggsave(file='C:/Documents and Settings/Russell/Desktop/VAT/Projects/Research/Conferences/1204CSBS/images/gworldwages.png')
